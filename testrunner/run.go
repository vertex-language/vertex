...
func execTest(tc TestCase, opts Options, compiler Compiler, stderr io.Writer) bool {
	...
	code := compiler.Compile(pkg, binPath, &compileErr)

	if tc.WantCompileErr {
		if code != 0 {
			fmt.Fprintf(os.Stdout, "ok    %s  (rejected as expected)\n", label)
			return true
		}
		fmt.Fprintf(os.Stdout, "FAIL  %s  (expected a compile error, but it compiled)\n", label)
